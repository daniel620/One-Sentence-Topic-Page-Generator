...
The revision loop has been removed. QA failures are now repaired
deterministically by generator.repair — no LLM re-composition.
"""
...
from generator.schemas import EvidenceAwareIA, EvidenceGraph, TopicPageData
...
def synthesize_topic_page(
    ...
    *,
    ...
    """Synthesize a topic page from evidence.

    page_mode:
      - "full": normal page with all supported claims
      - "degraded": evidence is thin — be conservative with hard facts,
        prefer timeline/entity/action sections over stat_grid, add
        qualifiers to metric claims, use fewer key facts
    """
    current_date = today or date.today()
    llm_client = client or _build_instructor_client()

    messages = build_stage3_messages(evidence_graph, ia, current_date, page_mode=page_mode)
